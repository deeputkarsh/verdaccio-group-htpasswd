 ...
 * @returns {object}
 */
export function parseHTPasswd(input: string, groupName: string, existing: {}): Record<string, any> {
  return input.split('\n').reduce((result, line) => {
    const args = line.split(':', 3);
    if (args.length > 1) {
      const [userId, passwd] = args;
      result[userId] = { passwd, groups: [groupName] };
      if (existing[userId]) {
        result[userId].groups = Array.from(new Set([...existing[userId].groups, groupName]));
      }
    }
    return result;
  }, {});
...
export function sanityCheck(
  ...
  if (existingUser) {
    const auth = verifyFn(password, existingUser.passwd);
    if (auth) {
